...
Wraps [lizard](https://github.com/terryyin/lizard) to produce GitLab-compatible
[Code Quality reports](https://docs.gitlab.com/ee/ci/testing/code_quality.html)
that surface complexity **regressions and improvements on MR diffs**, not just
static threshold violations.

## Why

The naive approach — flag every function above a CCN threshold — produces a
report full of pre-existing issues that look identical in every pipeline.
GitLab sees no change between the MR and target branch reports, so nothing
appears in the Code Quality widget.
...
Runs lizard on the merge-base versions of files changed in the current branch,
producing a baseline CSV for delta comparison.
